...
for ($i = 1; $i <= $prix; $i++) {
    ...
    $pdf->SetX($pdf->GetX() + -5);
    $pdf->MultiCell(130, 5, $prix_palmares, 0, 'C', 0, 0, '', '', true, 0, false, false, 40, '');
    // Récupérer la hauteur de la cellule précédente
    $previousCellHeight = $pdf->getLastH();

    $pdf->SetFont('times', 'B', 15);
    $pdf->SetY($pdf->GetY() + 1 + $previousCellHeight); // Ajouter la hauteur de la cellule précédente
    $pdf->SetX($pdf->GetX() + -8);
